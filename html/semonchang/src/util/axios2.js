...
const httpRequestor = {
  ...
  //上传文件超时时间
  defaultUploadFileTimeOut: 120000
};

const axiosInstance = axios.create({
  baseURL: httpRequestor.baseURL,
  timeout: httpRequestor.defaultTimeOut
  ...
  remove(config) {
    if (
      ...
      if (index >= 0) {
        ...
        console.error(
          `The url [${url}] is not found in urlRecorder`,
          this.urlList
        );
      }
    }
 ...
 * @param {string} url
 * @param {object?} data={}
 * @param {[int]?} codeWhiteList 如果返回的是这些错误码就不弹提示
...
httpRequestor.post = function postJson(
  url,
  data = {},
  { codeWhiteList, timeout, withCredentials } = {}
) {
  return commonAjax({
    method: "POST",
    url,
    ...
    timeout: timeout || httpRequestor.defaultTimeOut,
    withCredentials:
      withCredentials !== undefined
        ? withCredentials
        : httpRequestor.withCredentials,
    headers: {
...
httpRequestor.uploadFile = function uploadFile(
  ...
  return commonAjax({
    ...
    onUploadProgress,
    codeWhiteList: codeWhiteList || [],
    timeout: timeout || httpRequestor.defaultUploadFileTimeOut,
    withCredentials:
      withCredentials !== undefined
        ? withCredentials
        : httpRequestor.withCredentials
 ...
 * @return {Promise} 返回一个promise对象。其中then方法传递回包中的data数据；catch事件则传递整个回包，其参数为{data:{},status{code:123,message:'xxx'}}
 */
httpRequestor.get = function get(
  url,
  params = {},
  { codeWhiteList, timeout, withCredentials } = {}
) {
...
function handleError(requestConfig, result) {
  ...
  // 没有设置白名单或白名单不包含该错误码，需要弹出默认toast
  const needDefaultHandler =
    requestConfig &&
    (!requestConfig.codeWhiteList.length ||
      !requestConfig.codeWhiteList.includes(err.code));
  if (
    requestConfig &&
    needDefaultHandler &&
    httpRequestor.defaultErrorHandler
  ) {
    err.url = requestConfig.url;
    // 默认的错误处理方法，一般是弹toast或大红条
    httpRequestor.defaultErrorHandler(err);
    err.processed = true;
  }
